...
def Evaluate(result_arr):
    ...
    for entry in result_arr:
        ...
        for i in range(len(word_match_list)):
            ...
            with open(str(tag)+"_ndcg.csv",'a') as filef:
                rd = csv.writer(filef)
                rd.writerow([lemma_match,word_match,n_dcsWords])
...
def main():
    ...
    for i in range(proc_count):
        vpid = i
        procs[i] = mp.Process(target = rank_test.pooled_Test, args = \
...
if __name__ == '__main__':
    ...
    setArgs(_tag, pc)
    for i in range(6):
        with open(str(tag)+"_ndcg.csv","w") as f:
            rd = csv.writer(f)
