...
def Evaluate(result_arr):
    ...
    for entry in result_arr:
        ...
        fully_Correct_l = 0
        fully_Correct_w = 0
        (word_match_list, lemma_match_list, n_dcsWords, n_output_nodes) = entry
        for i in range(len(word_match_list)):
            word_match=word_match_list[i]
            lemma_match=lemma_match_list[i]
            ...
            with open(str(tag)+"result_sasi"+str(i)+".csv",'a') as filef:
                rd = csv.writer(filef)
            ...
            # rl = np.mainean(np.array(recalls))
...
def main():
    ...
    modelList = {
        'PR2': 'outputs/train_{}/nnet.p'.format('t2788294192566'),
        'BR2': 'outputs/train_{}/nnet.p'.format('t2789415023871'),
        'PM2': 'outputs/train_{}/nnet.p'.format('t2753954441900'),
        'BM2': 'outputs/train_{}/nnet.p'.format('t4520394296121'),
        'PR3': 'outputs/train_{}/nnet.p'.format('t2761370242287'),
        'BR3': 'outputs/train_{}/nnet.p'.format('t2779114903467'),
        'PM3': 'outputs/train_{}/nnet.p'.format('t2756013734745'),
        'BM3': 'outputs/train_{}/nnet.p'.format('t2993066720570')
    }
    ...
    for i in range(proc_count):
        vpid = i
        procs[i] = mp.Process(target = avg_test.pooled_Test, args = \
